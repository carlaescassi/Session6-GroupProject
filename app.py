...
default_b = "Pclass" if "Pclass" in categorical_cols else categorical_cols[0]
...
palette = st.sidebar.selectbox(
    ...
    ["deep", "pastel", "muted", "dark", "colorblind"]
)
...
chart_b_col = st.sidebar.selectbox(
    "Chart B variable",
    categorical_cols,
    index=categorical_cols.index(default_b)
)
...
def show_chart(dataframe, group_col, target_col, title):
    plot_df = prepare_data(dataframe, group_col, target_col)

    fig, ax = plt.subplots()
    sns.barplot(data=plot_df, x=group_col, y=target_col, hue=group_col, palette=palette, legend=False, ax=ax)
    ax.set_title(title)
...
if st.session_state.chart_shown == "A":
    ...
    show_chart(df, chart_a_col, target_col, f"Chart A: {target_col} by {chart_a_col}")

elif st.session_state.chart_shown == "B":
    st.success("Chart B selected")
    show_chart(df, chart_b_col, target_col, f"Chart B: {target_col} by {chart_b_col}")
